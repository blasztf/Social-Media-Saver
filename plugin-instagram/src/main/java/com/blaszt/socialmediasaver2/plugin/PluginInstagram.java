...
public class PluginInstagram extends Plugin {
    private PluginStories mPluginStories;
    ...
    public String[] getMediaURLs(String url) {
        ...
        if (mPluginStories.isURLValid(url)) {
...
//            log.writeLog("Module :: It is not stories!");
            try {
                String response = getPluginNet().getResponse(
                        url,
                        new PluginNet.Config()
                        .addData("__a", "1")
                );
                JsonObject container = JsonParser.parseString(response).getAsJsonObject();
    ...
    public boolean isURLValid(String url) {
        return url.matches("https?://(www\\.)?instagram\\.com/p/[a-zA-Z0-9_\\-]{11,}/?.*") || mPluginStories.isURLValid(url);
    }
    ...
    private String[] getMediaCollections(JsonObject container) {
        ...
        // more than one photo or video on the post
        else {
            JsonArray array;
            JsonObject node;
